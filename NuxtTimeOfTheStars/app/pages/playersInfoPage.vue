<template>
  ...
  >
    ...
    <div class="max-w-7xl mx-auto relative z-10">
      ...
      >
        <div class="text-center bg-white dark:bg-gray-900 rounded-3xl p-12 shadow-2xl">
          <div class="text-6xl mb-4">😕</div>
          <h1 class="text-4xl font-bold text-gray-800 dark:text-gray-200 mb-4">Игрок не найден</h1>
        ...
        >
          ...
          >
            ...
            <div class="text-center text-white relative z-10">
              <div class="text-8xl md:text-9xl mb-6 animate-bounce" style="animation-duration: 3s">
                🏒
              </div>
              <h1 class="text-4xl md:text-6xl font-black mb-4 tracking-wider">{{ player.name }}</h1>
              <div
            ...
            >
              <span class="text-white font-bold flex items-center space-x-2">
            ...
            >
              <span class="text-white text-sm font-semibold">
                🔥 {{ player.goals + player.assists }} очков в сезоне
              </span>
            </div>
          </div>
          ...
          <div class="p-8 md:p-12">
            ...
            <!-- Вкладки статистики -->
            <div class="mb-8">
              <div class="flex flex-wrap gap-3 mb-6 p-2 bg-gray-100 dark:bg-gray-800 rounded-2xl">
                <button
              ...
              >
                ...
                >
                  <div :class="`text-3xl font-black text-${stat.color}-600 mb-2`">
                    {{ stat.value }}
                  </div>
              ...
              >
                ...
                >
                  <h4 :class="`font-bold text-${stat.color}-600 mb-3 text-lg`">
                    {{ stat.label }}
                  </h4>
                  <div :class="`text-3xl font-black text-${stat.color}-700 mb-2`">
                    {{ stat.value }}
                  </div>
              ...
              <!-- Последние игры -->
              <div v-if="activeStatTab === 'recent'" class="space-y-4">
                <div
                  ...
                  :class="{
                    ...
                    'border-red-500 bg-red-50/50 dark:bg-red-900/20': !game.result.startsWith('W'),
                  }"
                >
                  <div class="flex justify-between items-center">
                    <div class="flex items-center space-x-4">
                      <div class="text-center">
                        <div class="text-sm text-gray-500 mb-1">{{ game.date }}</div>
                        <div class="font-bold">vs {{ game.opponent }}</div>
                      </div>
                      <div
                        class="px-3 py-1 rounded-full text-sm font-bold"
                        :class="{
                          'bg-green-100 text-green-700': game.result.startsWith('W'),
                          'bg-red-100 text-red-700': !game.result.startsWith('W'),
                        }"
                      >
                        {{ game.result }}
                      </div>
                    ...
                    <div class="flex items-center space-x-6 text-center">
                      <div>
                        <div class="text-2xl font-bold text-blue-600">{{ game.goals }}</div>
                        <div class="text-xs text-gray-500">⚽ Голы</div>
                      </div>
                      <div>
                        <div class="text-2xl font-bold text-red-600">{{ game.assists }}</div>
                        <div class="text-xs text-gray-500">🎯 Передачи</div>
                      </div>
                      <div>
                        <div class="text-2xl font-bold text-yellow-600">{{ game.rating }}</div>
              ...
              >
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                  <div class="bg-white/50 dark:bg-gray-800/50 rounded-2xl p-4">
                    <strong class="text-blue-600">Место рождения:</strong> {{ player.birthPlace }}
                  </div>
                  <div class="bg-white/50 dark:bg-gray-800/50 rounded-2xl p-4">
                    <strong class="text-red-600">Позиция:</strong> {{ player.position }}
                  </div>
                </div>
              </div>
            </div>
                ...
                >
                  <div class="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <p class="text-sm text-yellow-600">Престижная награда</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="mb-12">
              ...
              <div class="overflow-x-auto">
                ...
                >
                  ...
                  >
                    <tr>
                      <th class="px-6 py-4 text-left font-bold">Сезон</th>
                      <th class="px-6 py-4 text-left font-bold">Команда</th>
                      <th class="px-6 py-4 text-center font-bold">И</th>
                      <th class="px-6 py-4 text-center font-bold">Г</th>
                      <th class="px-6 py-4 text-center font-bold">П</th>
                      <th class="px-6 py-4 text-center font-bold">О</th>
                      <th class="px-6 py-4 text-center font-bold">Г/И</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr
                      v-for="(stat, index) in player.careerStats"
                      :key="index"
                      :class="{
                    ...
                    >
                      <td class="px-6 py-4 font-bold text-blue-600">{{ stat.season }}</td>
                      <td class="px-6 py-4 font-semibold">{{ stat.team }}</td>
                      <td class="px-6 py-4 text-center font-bold">{{ stat.games }}</td>
                      ...
                      <td class="px-6 py-4 text-center font-bold text-purple-600">
                        {{ (stat.goals / stat.games).toFixed(2) }}
                      </td>
                    </tr>
                  </tbody>
                </table>
            ...
            <div class="text-center">
              <router-link to="/">
                ...
                >
                  ...
                  ></div>
                  <span class="relative z-10 flex items-center space-x-3 text-lg">
                    <span>🏒</span>
...
const quickStats = computed(() => [
  ...
  { label: '📄 Контракт', value: `до ${player.value.contract}`, color: 'indigo' },
])
...
const currentSeasonStats = computed(() => [
  ...
  { label: '🎯 Передачи', value: player.value.assists, color: 'red', trend: '+3' },
  { label: '🏆 Очки', value: player.value.points, color: 'green', trend: '+8' },
  { label: '🎮 Игры', value: player.value.gamesPlayed, color: 'purple', trend: '25/30' },
  { label: '📈 +/-', value: player.value.plusMinus, color: 'yellow', trend: '↗️' },
  { label: '⏱️ Штраф', value: `${player.value.penaltyMinutes} мин`, color: 'orange', trend: '-2' },
  { label: '🔥 ПП голы', value: player.value.powerPlayGoals, color: 'indigo', trend: '+2' },
  { label: '⚡ МН голы', value: player.value.shortHandedGoals, color: 'pink', trend: '+1' },
])
